...
// ✅ Move these ABOVE the `/:id` route
// Auto Complete Food Items
foodItemsRouter.get("/auto-complete", auth, async (req, res) => {
  try {
    const { query } = req.query;
    // When query is empty, return no food items
    if (!query) {
...
// Search Food Items
foodItemsRouter.get("/search", auth, async (req, res) => {
  try {
    const { query } = req.query;
    const foodItems = await FoodItem.find({
      name: { $regex: query, $options: "i" },
    ...
    res.status(200).json({
      message: "Food items fetched successfully",
      foodItems: foodItems.map((foodItem) => foodItem.name),
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
foodItemsRouter.patch("/:id", auth, async (req, res) => {
  try {
    ...
    const foodItem = await FoodItem.findByIdAndUpdate(
      id,
      { name, price, isOffer, offePercentage, foodTime, foodType, foodDescription, image },
      { new: true }
    );
